...
fft_plan fft_plan_dft_1d(int n, fft_complex *in, fft_complex *out, int sign, unsigned int flags)
{
	...
	output.sign		= sign;
	output.flags	= flags;
	output.input	= (double *)_aligned_malloc(sizeof(double) * n * 2, 16);
	output.ip		= (int *)malloc(sizeof(int) * n);
	output.w		= (double *)malloc(sizeof(double) * n * 5 / 4);
...
fft_plan fft_plan_dft_c2r_1d(int n, fft_complex *in, double *out, unsigned int flags)
{
	...
	output.flags	= flags;
	output.input = (double *)_aligned_malloc(sizeof(double) * n, 16);
	output.ip		= (int *)malloc(sizeof(int) * n);
	output.w		= (double *)malloc(sizeof(double) * n * 5 / 4);
...
void fft_execute(const fft_plan p)
{
	...
	else // ifft
	{
		...
		else // c2c
		{
			...
			for(i = 0;i < p.n;i++)
			{
//				p.c_out[i][0]   = p.input[i*2]*2.0;
//				p.c_out[i][1]   = p.input[i*2+1]*2.0;
				p.c_out[i][0]   = p.input[i*2];
				p.c_out[i][1]   = p.input[i*2+1];
			}
		}
    ...
    if (l == 8) {
        for (k = 0; k < m; k++) {
            for (j = 0; j < k; j++) {
                j1 = 4 * j + 2 * ip[m + k];
                k1 = 4 * k + 2 * ip[m + j];
                xri = _mm_load_pd((double *)&a[j1]);	//xr = a[j1];
											//xi = a[j1 + 1];
                yri = _mm_load_pd(&a[k1]);	//yr = a[k1];
											//yi = a[k1 + 1];
				_mm_store_pd(&a[j1], yri);	//a[j1] = yr;
                							//a[j1 + 1] = yi;
                _mm_store_pd(&a[k1], xri);	//a[k1] = xr;
                							//a[k1 + 1] = xi;
                j1 += nm;
                k1 += 2 * nm;
                xr = a[j1];
